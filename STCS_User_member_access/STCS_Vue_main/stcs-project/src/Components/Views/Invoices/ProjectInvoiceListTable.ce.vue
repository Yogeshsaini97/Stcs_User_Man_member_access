...
const dropdownOptions= ["All",'Paid','Overdue','Partially Paid', 'Cancelled', 'Requested modification' ];
...
const selectedOption = ref(dropdownOptions[0]);
    ...
    async function selectOption(option) {
      ...
      let newUrl;
      newUrl=props.hostUrl + ` and invoiceStatus eq '${option}'`;
    ...
    <table class="table font-weight-600">
        ...
        <tbody v-if="userList.length > 0">
          <tr  v-for="item in userList" :key="item.id">
            <td><span class="badge rounded-pill bg-light-success text-dark font-10"><i class='fa-solid fa-circle font-5 align-middle mr-2'></i>{{ item.invoiceStatus }}</span></td>
            <td>{{ item.referenceID }}</td>
            <td>{{ item.invoiceValue }}<br /></td>
            <td>{{ item.creator.name }}<br /><span class="font-weight-400">STCS - {{ item.creator.contentType }}</span></td>
            <td>{{ ChangeDateFormat(item.dueDate) }}</td>
